...
class WriteCF(object):
    ...
    def _writecf(self):
        ...
        tmpdct = {"운영_기초"     : {"기초잔액": oprtg.df.bal_strt},
                  "CashIn"      : wb.extnddct(
                                  ...
                                  {"Fund"   : fund.cmn.ntnl.df.amt_out},
                                  {"Loan_"+key: item.ntnl.df.amt_out for key, item in loan.dct.items()},
                                  {"Mrtg_"+key: item.ntnl.df.amt_out for key, item in mrtg.dct.items()},
                                  {"Sales"  : sales.df.amt_out}
                                  ),
                  "상환_loan"    : wb.extnddct(
                                  {"Loan_"+key: item.ntnl.df.amt_in for key, item in loan.dct.items()},
                                  {"Mrtg_"+key: item.ntnl.df.amt_in for key, item in mrtg.dct.items()},
                                  {"Fund"   : fund.cmn.ntnl.df.amt_in}
                                  ),
                  "운영_유입"     : {"현금유입": oprtg.df.amt_in},
                  "조달비용"      : wb.extnddct(
                                  {"Loan_"+item.title: item.df.amt_in for key, item in loancst.dct.items()},
                                  {"Mrtg_"+item.title: item.df.amt_in for key, item in mrtgcst.dct.items()},
                                  {"Fund_"+item.title: item.df.amt_in for key, item in fundcst.dct.items()}
                                  ),
                  "금융비용"      : wb.extnddct(
                  ...
                  "담보대출"      : wb.extnddct(
                                  {"Fee_"+key: item.fee.df.amt_in for key, item in mrtg.dct.items()},
                                  {"IR_"+key: item.IR.df.amt_in for key, item in mrtg.dct.items()},
                                  {"Fob_"+key: item.fob.df.amt_in for key, item in mrtg.dct.items() if item.rate_fob > 0}),
                  "펀드배당"      : wb.extnddct(
                                  {"Fee_"+key: item.fee.df.amt_in for key, item in fund.dct.items()},
                                  {"IR_"+key: item.IR.df.amt_in for key, item in fund.dct.items()},
                                  {"Fob_"+key: item.fob.df.amt_in for key, item in fund.dct.items() if item.rate_fob > 0}),
                  }
        ...
        tmpdct = wb.extnddct(tmpdct,
                 {"상환_equity"  : {"Equity" : equity.ntnl.df.amt_in},
                  "운영_유출"  : {"현금유출" : oprtg.df.amt_out},
    ...
    """         
        ...
        for rnk in sorted(loan.rnk, reverse=False):
            ...
            col += 1
            ws.write_string(row+1, col, "amt_out", self.fmt_bold)
            ws.write_column(row+2, col, loan[rnk].ntnl.df.amt_out, self.fmt_num1)
            col += 1
            ws.write_string(row+1, col, "amt_in", self.fmt_bold)
            ws.write_column(row+2, col, loan[rnk].ntnl.df.amt_in, self.fmt_num1)
            col += 1
            ...
            ws.write_string(row+1, col, "amt_in", self.fmt_bold)
            ws.write_column(row+2, col, loan[rnk].fee.df.amt_in, self.fmt_num1)
            col += 1
            ws.write_string(row+1, col, "bal_end", self.fmt_bold)
            ...
            ws.write_string(row+1, col, "amt_in", self.fmt_bold)
            ws.write_column(row+2, col, loan[rnk].IR.df.amt_in, self.fmt_num1)
            col += 1
            ws.write_string(row+1, col, "bal_end", self.fmt_bold)
            ...
            if loan[rnk].rate_fob > 0:
                ...
                ws.write_string(row+1, col, "amt_in", self.fmt_bold)
                ws.write_column(row+2, col, loan[rnk].fob.df.amt_in, self.fmt_num1)
                col += 1
                ws.write_string(row+1, col, "bal_end", self.fmt_bold)
            ...
            ws.write_string(row+1, col, "amt_out", self.fmt_bold)
            ws.write_column(row+2, col, loan[rnk].df.amt_out, self.fmt_num1)
            col += 1
            ws.write_string(row+1, col, "amt_in", self.fmt_bold)
            ws.write_column(row+2, col, loan[rnk].df.amt_in, self.fmt_num1)
            col += 1
            ws.write_string(row+1, col, "bal_end", self.fmt_bold)
        ...
        ws.write_string(row+1, col, "amt_out", self.fmt_bold)
        ws.write_column(row+2, col, equity.df.amt_out, self.fmt_num1)
        col += 1
        ws.write_string(row+1, col, "amt_in", self.fmt_bold)
        ws.write_column(row+2, col, equity.df.amt_in, self.fmt_num1)
        col += 1
        ws.write_string(row+1, col, "bal_end", self.fmt_bold)
    ...
    def _writecst(self):
        ...
        for key, item in cost._dct.items():
            ...
            for dct in cstlst:
                ...
                amt_ttl = dct["amtttl"]
                amt_intl = dct["account"].df.amt_in[idx_intl]
                amt_loan = sum(dct["account"].df.amt_in[idx_loan])
                amt_rsrv = sum(dct["account"].df.amt_in[idx_rsrv])
        ...
        for rnk in sorted(loan.rnk, reverse=False):
            ...
            lst_amt = loan[rnk].fee.df.amt_in
            amt_ttl = sum(lst_amt)
            amt_intl = lst_amt[idx_intl]
            amt_loan = sum(lst_amt[idx_loan])
            amt_rsrv = sum(lst_amt[idx_rsrv])
        ...
        for rnk in sorted(loan.rnk, reverse=False):
            if loan[rnk].rate_fob > 0:
                ...
                lst_amt = loan[rnk].fob.df.amt_in
                amt_ttl = sum(lst_amt)
                amt_intl = lst_amt[idx_intl]
                amt_loan = sum(lst_amt[idx_loan])
                amt_rsrv = sum(lst_amt[idx_rsrv])
        ...
        for rnk in sorted(brgl.rnk, reverse=False):
            ...
            lst_amt = brgl[rnk].fee.df.amt_in
            amt_ttl = sum(lst_amt)
            amt_intl = lst_amt[idx_intl]
            amt_loan = sum(lst_amt[idx_loan])
            amt_rsrv = sum(lst_amt[idx_rsrv])
        ...
        for rnk in sorted(brgl.rnk, reverse=False):
            if brgl[rnk].rate_fob > 0:
                ...
                lst_amt = brgl[rnk].fob.df.amt_in
                amt_ttl = sum(lst_amt)
                amt_intl = lst_amt[idx_intl]
                amt_loan = sum(lst_amt[idx_loan])
                amt_rsrv = sum(lst_amt[idx_rsrv])
